...
import {
  Body, Container, Head, Heading, Hr, Html, Img, Link, Preview, Section, Text, Button,
} from '@react-email/components';

export type MagicLinkKind = 'signin' | 'approved' | 'approved-team' | 'team-assigned';
...
const COPY: Record<MagicLinkKind, {
  preview: string;
  tagText: string;
  tagBg: string;
  tagFg: string;
  ...
  signin: {
    preview: 'Trykk på lenken for å logge inn på The Grandest Slam. Lenken utløper om 15 minutter.',
    tagText: 'Magisk lenke',
    tagBg: mustard,
    tagFg: ink,
    heading: () => <>Velkommen <span style={{ color: terracotta }}>tilbake</span>.</>,
    body: () => 'Trykk på knappen under for å logge inn. Lenken er gyldig i 15 minutter — etter det må du be om en ny.',
    cta: 'Logg inn →',
  },
  approved: {
    ...
    tagBg: teal,
    tagFg: cream50,
    heading: () => <>Du er <span style={{ color: teal }}>inne</span>.</>,
    body: () => 'En organisator har godkjent påmeldingen din. Trykk under for å logge inn. Vi setter deg på et lag så snart vi har en god makker til deg — du får beskjed her når det er gjort.',
    cta: 'Logg inn →',
  },
  'approved-team': {
    ...
    tagBg: plum,
    tagFg: cream50,
    heading: (p) => <><span style={{ color: plum }}>{p.teamName ?? '…'}</span> er godkjent.</>,
    body: (p) => (
      <>
        Laget <strong>{p.teamName ?? '…'}</strong> ({p.name} + {p.partnerName ?? '…'}) er godkjent og offisielt med. Trykk under for å logge inn og se hva som skjer videre.
      </>
    ),
    cta: 'Logg inn →',
  },
  'team-assigned': {
    ...
    tagBg: terracotta,
    tagFg: cream50,
    heading: () => <>Laget <span style={{ color: terracotta }}>ditt</span> er klart.</>,
    body: (p) => (
      <>
        Hei {p.name} — du er paret med <strong>{p.partnerName ?? '…'}</strong>
        {p.partnerSkill ? <> ({p.partnerSkill})</> : null}.
        Trykk under for å se kampoppsettet og bekrefte at dere er klare for helgen.
      </>
    ),
...
export const MagicLinkEmail: React.FC<MagicLinkEmailProps> = ({
  kind, name, magicUrl, teamName, partnerName, partnerSkill,
  logoUrl = 'https://www.thegrandestslam.no/The_Grandest_Slam.webp',
}) => {
  const c = COPY[kind];
  const props: MagicLinkEmailProps = { kind, name, magicUrl, teamName, partnerName, partnerSkill };
  return (
    <Html lang="no">
      <Head>
      ...
      <Body style={{ margin: 0, padding: 0, background: cream, fontFamily: fontSans, color: ink }}>
        <Container style={{ width: '100%', maxWidth: 560, margin: '0 auto', padding: '32px 16px' }}>
          ...
          <Section style={{
            ...
            <Text style={{ margin: '0 0 28px', fontSize: 16, lineHeight: 1.55, color: ink }}>
              {c.body(props)}
            </Text>

            <Button href={magicUrl} style={{
              display: 'inline-block', padding: '16px 28px',
              background: terracotta, color: cream50,
              fontFamily: fontSans, fontWeight: 700, fontSize: 16, letterSpacing: '0.02em',
              border: `2px solid ${ink}`, borderRadius: 9999,
              boxShadow: `4px 4px 0 ${ink}`, textDecoration: 'none',
            }}>
              {c.cta}
            </Button>

            <Text style={{ margin: '24px 0 0', fontSize: 13, lineHeight: 1.5, color: 'rgba(26,20,16,0.7)' }}>
              Hvis knappen ikke virker, lim denne lenken inn i nettleseren:<br />
              <Link href={magicUrl} style={{ color: teal, wordBreak: 'break-all', textDecoration: 'underline' }}>{magicUrl}</Link>
            </Text>
          </Section>
          ...
          <Section style={{ padding: '28px 0 8px', textAlign: 'center' }}>
            <Text style={{ margin: '0 0 12px', fontSize: 13, lineHeight: 1.55, color: 'rgba(26,20,16,0.7)' }}>
              <Link href="https://www.thegrandestslam.no/leaderboard" style={{ color: ink, fontWeight: 700, textDecoration: 'underline' }}>
                Live resultatliste →
              </Link>
            </Text>
            <Text style={{ margin: 0, fontSize: 12, lineHeight: 1.55, color: 'rgba(26,20,16,0.55)' }}>
              Ved å delta samtykker du i å spille hardt, tape verdig og bekrefte motstandernes poeng ærlig.
            </Text>
          </Section>
        </Container>
      </Body>
...
export function subjectFor(kind: MagicLinkKind): string {
  switch (kind) {
    case 'signin':        return 'Velkommen tilbake til slammet — trykk her ↓';
    case 'approved':      return 'Du er godkjent — logg inn for å komme i gang ↓';
    case 'approved-team': return 'Laget er godkjent — logg inn for å komme i gang ↓';
    case 'team-assigned': return 'Laget ditt er klart — møt makkeren din ↓';
  }
}
