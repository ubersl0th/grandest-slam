...
const COPY: Record<SubmissionStatusKind, KindConfig> = {
	"received-solo": {
		...
		heading: (p) => (
			<>
				Takk, <span style={{ color: teal }}>{p.name}</span>!
			</>
		),
		body: () => (
			<>
				...
				>
					Vi har mottatt påmeldingen din. En organisator vurderer den i løpet av
					kort tid — du får beskjed på e-post så snart vi har bestemt oss.
				</Text>
				<Text
					style={{
						margin: 0,
				...
				>
					Du trenger ikke å foreta deg noe nå. Spar denne e-posten så du har den
					om du lurer på status.
				</Text>
			</>
		),
	},
	"received-team": {
		preview:
			"Vi har mottatt påmeldingen for laget. En organisator vurderer den nå.",
		tagText: "Lag mottatt",
		tagBg: mustard,
		tagFg: ink,
		isRejected: false,
		heading: (p) => (
			<>
				<span style={{ color: plum }}>{p.teamName ?? "…"}</span> er meldt på.
			</>
		),
		body: (p) => (
			<>
				...
				>
					Hei {p.name} — vi har mottatt påmeldingen for laget{" "}
					<strong>{p.teamName ?? "…"}</strong> ({p.name} +{" "}
					{p.partnerName ?? "…"}). En organisator vurderer den i løpet av kort
					tid, og dere får begge beskjed på e-post så snart vi har bestemt oss.
				</Text>
				<Text
					style={{
						margin: 0,
	...
	"rejected-solo": {
		...
		heading: () => (
			<>
				...
				<span style={{ color: terracottaDark }}>plass</span>.
			</>
		),
		body: (p) => (
			<Text
			...
			>
				Hei {p.name} — vi har gått gjennom påmeldingen din, og dessverre har vi
				ikke plass til deg i årets slam.
			</Text>
		),
	},
	"rejected-team": {
		...
		body: (p) => (
			...
			>
				Hei {p.name} — vi har gått gjennom påmeldingen for laget{" "}
				<strong>{p.teamName ?? "…"}</strong>, og dessverre har vi ikke plass til
				dere i årets slam.
			</Text>
		),
	},
...
export const SubmissionStatusEmail: React.FC<SubmissionStatusEmailProps> = (
	props,
) => {
	const {
		kind,
				...
				>
					...
					>
								...
								<tbody>
									<tr>
										...
										>
											...
											>
												Beskjed fra organisatorene
											</div>
											<div
												style={{ fontSize: 15, lineHeight: 1.55, color: ink }}
											>
												{reason}
											</div>
										</td>
						...
						{c.isRejected && (
							...
							>
								Vi setter pris på at du ville være med, og håper å se deg neste
								år. Har du spørsmål, svar gjerne på denne e-posten — vi leser
								alt.
							</Text>
						)}
					</Section>
...
export function statusSubjectFor(
	...
	switch (kind) {
		case "received-solo":
			return "Påmelding mottatt — vi vurderer den nå";
		case "received-team":
			return `Påmelding mottatt — vi vurderer ${p.teamName ?? "laget"}`;
		case "rejected-solo":
			return "Påmeldingen din — beslutning fra organisatorene";
		case "rejected-team":
			return "Laget deres — beslutning fra organisatorene";
	}
}
