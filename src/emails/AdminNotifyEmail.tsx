...
const DataRow: React.FC<{ label: string; children: React.ReactNode; first?: boolean }> = ({ label, children, first }) => (
  <tr>
    <td style={{
      padding: '10px 18px', fontSize: 13, color: muted, verticalAlign: 'top',
      borderTop: first ? 'none' : `1px solid ${hairline}`, width: '35%',
    }}>{label}</td>
    <td style={{
      padding: '10px 18px', fontSize: 15, color: ink,
      borderTop: first ? 'none' : `1px solid ${hairline}`,
    }}>{children}</td>
  </tr>
);

const SectionHeader: React.FC<{ children: React.ReactNode; tinted?: boolean; topBorder?: boolean }> = ({ children, tinted, topBorder }) => (
  <tr>
    <td colSpan={2} style={{
      ...
      <span style={{ fontFamily: fontDisplay, fontSize: 13, letterSpacing: '0.14em', color: ink }}>{children}</span>
    </td>
  </tr>
);

export const AdminNotifyEmail: React.FC<AdminNotifyEmailProps> = ({
  kind, name, email, skill, signedUpAt, adminUrl,
  teamName, partnerName, partnerEmail, partnerSkill,
        ...
        <Container style={{ width: '100%', maxWidth: 560, margin: '0 auto', padding: '32px 16px' }}>
          ...
          <Section style={{
            ...
            <div style={{ marginBottom: 20 }}>
              <span style={{
                display: 'inline-block', padding: '6px 12px', background: ink, color: cream50,
            ...
            <Heading as="h1" style={{
              ...
              fontSize: 48, lineHeight: 0.9, letterSpacing: '-0.01em', color: ink,
            }}>
              <span style={{ color: heroAccent }}>{hero}</span> venter på godkjenning.
            </Heading>

            <Text style={{ margin: '0 0 24px', fontSize: 16, lineHeight: 1.55, color: 'rgba(26,20,16,0.85)' }}>
              {isTeam
            ...
            <table role="presentation" width="100%" cellPadding={0} cellSpacing={0} style={{
              margin: '0 0 28px', border: `2px solid ${ink}`, borderRadius: 12, background: cream,
            }}>
              <tbody>
                {isTeam ? (
                  <>
                    <SectionHeader>LAG</SectionHeader>
                    <DataRow label="Lagnavn" first><strong>{teamName}</strong></DataRow>
                    <DataRow label="Meldt på">{signedUpAt}</DataRow>

                    <SectionHeader tinted topBorder>SPILLER 1</SectionHeader>
                    <DataRow label="Navn" first><strong>{name}</strong></DataRow>
                    <DataRow label="E-post"><Link href={`mailto:${email}`} style={{ color: teal, textDecoration: 'underline' }}>{email}</Link></DataRow>
                    <DataRow label="Nivå">{skill}</DataRow>

                    <SectionHeader tinted topBorder>SPILLER 2</SectionHeader>
                    <DataRow label="Navn" first><strong>{partnerName}</strong></DataRow>
                    <DataRow label="E-post"><Link href={`mailto:${partnerEmail}`} style={{ color: teal, textDecoration: 'underline' }}>{partnerEmail}</Link></DataRow>
            ...
            <Text style={{ margin: '24px 0 0', fontSize: 13, lineHeight: 1.5, color: muted }}>
              Hvis knappen ikke virker, lim denne lenken inn i nettleseren:<br />
              <Link href={adminUrl} style={{ color: teal, wordBreak: 'break-all', textDecoration: 'underline' }}>{adminUrl}</Link>
            </Text>
          </Section>
...
export function adminSubjectFor(p: { kind: AdminNotifyKind; name: string; teamName?: string }): string {
  return p.kind === 'team'
